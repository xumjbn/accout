...
- 👨‍👩‍👧 **家庭共享**：创建家庭生成邀请码，家人加入后账单云端同步（预算/资产仍归各自设备）
...
1. 开发者工具 → 云开发 → 开通（需正式 AppID，`project.config.json` 的 appid 目前为空）
2. 创建三个集合：`families`、`family_members`、`family_tx`
3. 三个集合权限暂设「所有用户可读写」（MVP 简化；正式上线应改为
   自定义安全规则 + 云函数校验成员身份，避免越权读写）

未开通时家庭页会显示配置指引，其余功能不受影响。

## 已知边界

- 家庭同步为「云端最后写入获胜」，无冲突合并；成员退出仅本机退出
- 客户端拉取云端账单按 20 条/页分页，账单量数千级别时进家庭页会稍慢
